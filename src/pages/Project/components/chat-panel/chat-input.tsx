...
import { MessageFileMeta } from '@/api/types'



interface ChatInputProps {
  onSendMessage: (text: string, metas: MessageFileMeta[]) => Promise<void>
  /** ส่งจาก suggestion: ทุกครั้งที่ seq เปลี่ยน จะใส่ข้อความลงช่องพิมพ์ */
  fillFromSuggestion?: { seq: number; text: string } | null
}

const CHAT_MESSAGE_INPUT_ID = "chat-panel-message-input"

export function ChatInput({ onSendMessage, fillFromSuggestion }: ChatInputProps) {
  ...
  useEffect(() => {
    if (!fillFromSuggestion) return
    setContent(fillFromSuggestion.text)
    queueMicrotask(() => {
      document.getElementById(CHAT_MESSAGE_INPUT_ID)?.focus()
    })
  }, [fillFromSuggestion?.seq, fillFromSuggestion?.text])
  ...
  return (
    <div
      ...
      className="relative z-10 shrink-0 border-t border-border bg-background p-4"
    >
